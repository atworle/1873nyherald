...
for(s in 1:length(sequences)){
 #AI was telling me to use seq_along instead of length but that literally didn't work?
    newurl[[index]]<-paste0("https://chroniclingamerica.loc.gov/search/pages/results/?date1=1870&date2=1875&searchType=advanced&language=&sequence=",sequences[s],"&lccn=sn83030313&proxdistance=5&rows=20&ortext=&proxtext=&phrasetext=&andtext=&dateFilterType=yearRange&page=", pages[i],"&sort=relevance&format=json")
